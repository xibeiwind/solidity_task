...
contract ChainlinkPriceOracle is IPriceOracle {
    ...
    event PriceCacheUpdated(address indexed token, uint256 price, uint256 timestamp);
     ...
     */
    function getETHPrice() external view override returns (uint256 price, uint256 timestamp) {
        return _getPriceFromFeed(_ethPriceFeed);
    }
     ...
     * @inheritdoc IPriceOracle
     */
    function getTokenPrice(address token) external view override returns (uint256 price, uint256 timestamp) {
        address priceFeed = _tokenPriceFeeds[token];
        require(priceFeed != address(0), "Price feed not set for token");
        
        return _getPriceFromFeed(priceFeed);
    }
     ...
     * @inheritdoc IPriceOracle
     */
    function isPriceFeedAvailable(address priceFeed) external view override returns (bool) {
        ...
        try AggregatorV3Interface(priceFeed).latestRoundData() returns (
            ...
            return answer > 0 && _isPriceFresh(priceFeed);
        } catch {
            return false;
        }
    }
     ...
     */
    function _getPriceFromFeed(address priceFeed) internal view returns (uint256 price, uint256 timestamp) {
        require(priceFeed != address(0), "Invalid price feed");
     ...
     */
    function _isCacheValid(PriceCache memory cache) internal view returns (bool) {
     ...
     * @dev 内部函数：检查价格是否新鲜
     * @param priceFeed 价格源地址
     * @return 如果价格新鲜返回true，否则返回false
     */
    function _isPriceFresh(address priceFeed) internal view returns (bool) {
        try AggregatorV3Interface(priceFeed).latestRoundData() returns (
            uint80,
            int256,
            uint256,
            ...
            return block.timestamp <= updatedAt + 2 hours; // 2小时内的价格视为新鲜
        } catch {
            return false;
        }
    }
     ...
     * @param timestamp 时间戳
     */
    function _updatePriceCache(address priceFeed, uint256 price, uint256 timestamp) internal {
     ...
     */
    function getPriceCache(address priceFeed) external view returns (uint256 price, uint256 timestamp, uint256 blockNumber) {
        PriceCache memory cache = _priceCache[priceFeed];
