...
```javascript
// 捐赠
await beggingContract.donate({ value: ethers.parseEther("0.1") });

// 查询捐赠
const donation = await beggingContract.getDonation(userAddress);
...
```javascript
// 转账
await myERC20.transfer(recipient, amount);

// 授权
await myERC20.approve(spender, amount);

// 委托转账
await myERC20.transferFrom(from, to, amount);

// 铸造新代币（仅所有者）
await myERC20.mint(to, amount);
```

### MyNFT 使用
```javascript
// 铸造 NFT（仅所有者）
const tokenId = await myNFT.mintNFT(to, "ipfs://Qm.../metadata.json");
